'use client'

import { useState } from 'react'
import useSWR from 'swr'
import Link from 'next/link'
import { getPeople } from '@/lib/api'

function SeniorityBadge({ seniority }: { seniority: string | null }) {
  if (!seniority) return null
  const colors: Record<string, string> = {
    c_suite: 'bg-purple-100 text-purple-800',
    founder: 'bg-purple-100 text-purple-800',
    vp: 'bg-indigo-100 text-indigo-700',
    director: 'bg-blue-100 text-blue-700',
    head: 'bg-sky-100 text-sky-700',
  }
  return (
    <span className={`px-1.5 py-0.5 rounded text-xs font-medium ${colors[seniority] ?? 'bg-gray-100 text-gray-600'}`}>
      {seniority.replace('_', '-')}
    </span>
  )
}

const LIMIT = 50

export default function PeoplePage() {
  const [nameQ, setNameQ] = useState('')
  const [titleQ, setTitleQ] = useState('')
  const [companyQ, setCompanyQ] = useState('')
  const [seniorityQ, setSeniorityQ] = useState('')
  const [page, setPage] = useState(0)

  // Debounced search params (applied on form submit)
  const [applied, setApplied] = useState({ name: '', title: '', company: '', seniority: '' })

  const { data, error } = useSWR(
    ['people', applied, page],
    () => getPeople({
      name: applied.name || undefined,
      title: applied.title || undefined,
      company: applied.company || undefined,
      seniority: applied.seniority || undefined,
      limit: LIMIT,
      offset: page * LIMIT,
    })
  )

  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault()
    setPage(0)
    setApplied({ name: nameQ, title: titleQ, company: companyQ, seniority: seniorityQ })
  }

  const handleClear = () => {
    setNameQ(''); setTitleQ(''); setCompanyQ(''); setSeniorityQ('')
    setPage(0)
    setApplied({ name: '', title: '', company: '', seniority: '' })
  }

  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-slate-900">People</h1>
        {data && (
          <span className="text-sm text-slate-500">{data.meta.total.toLocaleString()} total</span>
        )}
      </div>

      {/* Search bar */}
      <form onSubmit={handleSearch} className="bg-white rounded-lg shadow p-4 mb-4">
        <div className="grid grid-cols-4 gap-3">
          <input
            type="text"
            placeholder="Name..."
            value={nameQ}
            onChange={(e) => setNameQ(e.target.value)}
            className="px-3 py-2 text-sm border border-gray-300 rounded text-slate-800 placeholder-slate-400"
          />
          <input
            type="text"
            placeholder="Title..."
            value={titleQ}
            onChange={(e) => setTitleQ(e.target.value)}
            className="px-3 py-2 text-sm border border-gray-300 rounded text-slate-800 placeholder-slate-400"
          />
          <input
            type="text"
            placeholder="Company..."
            value={companyQ}
            onChange={(e) => setCompanyQ(e.target.value)}
            className="px-3 py-2 text-sm border border-gray-300 rounded text-slate-800 placeholder-slate-400"
          />
          <select
            value={seniorityQ}
            onChange={(e) => setSeniorityQ(e.target.value)}
            className="px-3 py-2 text-sm border border-gray-300 rounded text-slate-700"
          >
            <option value="">All seniority</option>
            <option value="c_suite">C-Suite</option>
            <option value="founder">Founder</option>
            <option value="vp">VP</option>
            <option value="director">Director</option>
            <option value="head">Head</option>
          </select>
        </div>
        <div className="flex gap-2 mt-3">
          <button type="submit" className="px-4 py-1.5 bg-blue-600 text-white text-sm rounded hover:bg-blue-700">
            Search
          </button>
          <button type="button" onClick={handleClear} className="px-4 py-1.5 text-slate-600 text-sm border border-gray-300 rounded hover:bg-gray-50">
            Clear
          </button>
        </div>
      </form>

      <div className="bg-white rounded-lg shadow overflow-hidden">
        {!data && !error ? (
          <div className="p-8 text-center text-slate-400 text-sm">Loading...</div>
        ) : error ? (
          <div className="p-8 text-center text-red-500 text-sm">Error loading people</div>
        ) : data.items.length === 0 ? (
          <div className="p-8 text-center text-slate-500 text-sm">No people found.</div>
        ) : (
          <>
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50">
                  <th className="px-5 py-3 text-left font-medium text-slate-500">Name</th>
                  <th className="px-5 py-3 text-left font-medium text-slate-500">Title</th>
                  <th className="px-5 py-3 text-left font-medium text-slate-500">Company</th>
                  <th className="px-5 py-3 text-left font-medium text-slate-500">Seniority</th>
                  <th className="px-5 py-3 text-left font-medium text-slate-500">Location</th>
                </tr>
              </thead>
              <tbody>
                {data.items.map((person) => (
                  <tr key={person.id} className="border-b border-gray-50 hover:bg-blue-50 cursor-pointer">
                    <td className="px-5 py-3">
                      <Link href={`/dashboard/people/${person.id}`} className="font-medium text-slate-900 hover:text-blue-700">
                        {person.full_name}
                      </Link>
                      {person.linkedin_url && (
                        <div className="text-xs text-slate-400 truncate max-w-[180px]">{person.linkedin_url}</div>
                      )}
                    </td>
                    <td className="px-5 py-3 text-slate-600">{person.current_title ?? '—'}</td>
                    <td className="px-5 py-3 text-slate-600">{person.current_company_name ?? '—'}</td>
                    <td className="px-5 py-3">
                      <SeniorityBadge seniority={person.seniority} />
                    </td>
                    <td className="px-5 py-3 text-slate-500 text-xs">{person.location ?? '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>

            {/* Pagination */}
            {data.meta.total > LIMIT && (
              <div className="px-5 py-3 border-t border-gray-100 flex items-center justify-between text-sm text-slate-600">
                <span>{data.meta.total.toLocaleString()} total · showing {page * LIMIT + 1}–{Math.min((page + 1) * LIMIT, data.meta.total)}</span>
                <div className="flex gap-2">
                  <button
                    onClick={() => setPage(p => Math.max(0, p - 1))}
                    disabled={page === 0}
                    className="px-3 py-1 border border-gray-200 rounded disabled:opacity-40 hover:bg-gray-50"
                  >
                    ← Prev
                  </button>
                  <button
                    onClick={() => setPage(p => p + 1)}
                    disabled={(page + 1) * LIMIT >= data.meta.total}
                    className="px-3 py-1 border border-gray-200 rounded disabled:opacity-40 hover:bg-gray-50"
                  >
                    Next →
                  </button>
                </div>
              </div>
            )}
          </>
        )}
      </div>
    </div>
  )
}
